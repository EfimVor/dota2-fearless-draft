/**
 * Dota 2 — Fearless Draft Client (v8 – герои не исчезают, добавлен зритель и панель истории)
...
function applyPickLocally(hero,attribute,team){
    ...
    if(state.step>=totalPicks()){
        ...
        stopTurnTimer();
        // Добавляем запись в историю при завершении игры
...
function addCurrentGameToHistory(){
    // Проверяем, не записана ли уже текущая игра
...
function applyNextGameLocally(){
    // Уже добавлена в history при завершении
...
function renderHistoryPanel(){
    ...
    let html = '';
    // Показываем последние игры сверху (в обратном порядке)
