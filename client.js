 ...
 * Мультиплеер через WebSocket. Авто-подключение к серверу на том же хосте.
 */
...
function deserializeState(s){state.seriesStarted=s.seriesStarted;state.currentGame=s.currentGame;state.availableHeroes=s.availableHeroes;state.phase=s.phase;state.step=s.step;state.currentTurn=s.currentTurn;state.bans=s.bans;state.picks=s.picks;state.currentGameBans=s.currentGameBans;state.currentGamePicks=s.currentGamePicks;state.seriesBannedHeroes=[...(s.seriesBannedHeroes||[])];state.seriesHistory=s.seriesHistory;state.radiantScore=s.radiantScore;state.direScore=s.direScore;state.waitingForWinner=s.waitingForWinner;}
...
function isMyCaptain(team){if(!state.captains[team])return true;if(state.captains[team].id==='remote')return false;return state.captains[team].id!=='remote';}
...
function applyBanLocally(hero,attribute,team){state.currentGameBans.push({hero,attribute,team});state.bans[team].push(hero);state.availableHeroes[attribute]=state.availableHeroes[attribute].filter(h=>h!==hero);state.step++;if(state.step>=totalBans()){state.phase='pick';state.step=0;state.currentTurn=config.pickOrder[0];renderAll();updateUI();showToast('Фаза банов завершена! Начинаются пики.','info');return;}state.currentTurn=getBanStepTeam(state.step);renderAll();updateUI();}
...
// ==================== RENDER ====================
function renderAll(){renderHeroPool();renderTeamBans('radiant');renderTeamBans('dire');renderTeamPicks('radiant');renderTeamPicks('dire');updateAttrCounts();updateGameBadge();updatePhaseBadge();updateScoreDisplay();renderCaptainBar();updateButtons();}
...
function updateUI(){updateTurnDisplay();updateButtons();updateGameBadge();updatePhaseBadge();updateScoreDisplay();}

function updateTurnDisplay(){const rT=document.getElementById('radiantTurn'),dT=document.getElementById('direTurn'),tI=document.getElementById('turnInfo');if(!state.seriesStarted){rT?.classList.add('hidden');dT?.classList.add('hidden');if(tI){tI.textContent='Подключитесь к комнате для начала';tI.className='turn-info';}resetPanelGlows();return;}if(state.phase==='complete'){rT?.classList.add('hidden');dT?.classList.add('hidden');if(tI){tI.textContent='Все пики сделаны — выберите победителя';tI.className='turn-info';}resetPanelGlows();return;}const isR=state.currentTurn==='radiant';rT?.classList.toggle('hidden',!isR);dT?.classList.toggle('hidden',isR);const pl=state.phase==='ban'?'Бан':'Пик';if(tI){tI.textContent=`${pl} — ход ${isR?'Radiant':'Dire'}`;tI.className='turn-info';if(state.phase==='ban')tI.classList.add('ban-phase-text');else if(isR)tI.classList.add('radiant-turn-text');else tI.classList.add('dire-turn-text');}const rP=document.getElementById('radiantPanel'),dP=document.getElementById('direPanel');if(rP&&dP){rP.style.boxShadow=isR?'0 0 30px var(--radiant-glow)':'none';dP.style.boxShadow=!isR?'0 0 30px var(--dire-glow)':'none';}}
