/* Dota 2 Fearless Draft - Game Logic (v18) */
var ALL_HEROES = {
...
function updateSpecUI(){
    ...
    if(!myTeam&&roomCode){
        ...
        if(cr)cr.style.display="";if(cd)cd.style.display="";
    }
}
...
function refresh(){renderHeroPool();renderTeamBans("radiant");renderTeamBans("dire");renderTeamPicks("radiant");renderTeamPicks("dire");renderHistoryPanel();updateAttrCounts();updateGameBadge();updatePhaseBadge();updateScoreDisplay();syncCaps();updateButtons();updateTimerDisplay();updateTurnDisplay();updateSpecUI();}
